   ...
   public:
    ...
    void push(const T &d) {
        try {
            ...
            cerr << endl << "\033[1;31mbad_alloc on stack caught: " << e.what() << "\033[0m" << endl;
        }
    }
    void gettop(T &v) {
        if (empty()) {
   ...
   public:
    ...
    void push_back(const T &d) {
        try {
            ...
            cerr << endl << "\033[1;31mbad_alloc on list caught: " << e.what() << "\033[0m" << endl;
        }
    }
...
inline void Solve(int task = 1) {
    ...
    if (task == 1 || task == 2) {
        ...
        List<string> ret;
        if (!Sol.InfixToPostfix(strList, ret)) return;
