...
class Maze {
    ...
    */
    bool DFS(int x, int y) {
        if (Goals <= 0) // no more goals
            return true;
        if (y < 0 || y >= graph.size()) return false;
        if (x < 0 || x >= graph[y].size()) return false;
        if (graph[y][x] == 'O') return false;
    ...
    bool DFS_shortestpath(int x, int y) {
        ...
        if (preV[y][x]) return false;
        if (!shortestpath.empty() && tmppath.size() >= shortestpath.size()) return false;
        // if (graph[y][x] != 'G')
        preV[y][x]++;
    ...
    */
    void PrintVisited() {
        for (int i = 0; i < y; ++i) {
            for (int j = 0; j < x; ++j) {
                if (visited[i][j] && graph[i][j] != 'G') // if true, the position has been visited
                    cout << 'V';
                else
                    cout << graph[i][j];
            }
    ...
    void PrintVisited3() {
        for (int i = 0; i < y; ++i) {
            for (int j = 0; j < x; ++j) {
                if (preV[i][j] && graph[i][j] != 'G') // if true, the position has been visited
                    cout << "\033[1;31mV\033[0m";
                else if (preV[i][j] && graph[i][j] == 'G')
                    cout << "\033[1;32mG\033[0m";
                else
                    cout << graph[i][j];
            }
...
int InputNumber() {
    ...
    try {
        ...
        return N;
    } catch (exception& e) { // if stoi throws exception, then treat the input as illegal
        return -1;
    }
