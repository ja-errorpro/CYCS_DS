    ...
    */
    ...
    void buildDeap() {
        ...
        for (int i = 1; i <= deap_len; i++) {
            VERBOSE(deap[i - 1], " ");
            if (floor(log2(i + 1)) != floor(log2(i + 2))) VERBOSE("\n");
        }
        cout << endl;
#endif
    }
    ...
    void buildMaxHeap() {
        ...
        VERBOSE(_data_arr[heap[0]].student_count, "\n");
        for (int i = 1; i < heap_len; i++) {
            VERBOSE(_data_arr[heap[i]].student_count, " ");
            if (floor(log2(i + 1)) != floor(log2(i + 2))) VERBOSE("\n");
        }
        cout << endl;
#endif
    ...
    void buildMinMaxHeap() {
        ...
        int heap_len = heap.size();
        VERBOSE(_data_arr[heap[0]].graduate_count, "\n");
        for (int i = 1; i < heap_len; i++) {
            VERBOSE(_data_arr[heap[i]].graduate_count, " ");
            if (floor(log2(i + 1)) != floor(log2(i + 2))) VERBOSE("\n");
        }
        cout << endl;
#endif
