...
public:
  Matrix(ullong a, ullong b, ullong c, ullong d) {
    num[0][0] = a;
    num[0][1] = b;
    num[1][0] = c;
    num[1][1] = d;
  ...
  Matrix operator*(const Matrix &m) {
    ...
    for (int i = 0; i < 2; ++i) {
      for (int j = 0; j < 2; ++j)
        for (int k = 0; k < 2; ++k)
          ret.num[i][j] += num[i][k] * m.num[k][j];
    }
    return ret;
  }
  ullong &operator()(ullong i, ullong j) { return num[i][j]; }
  ullong operator()(ullong i, ullong j) const { return num[i][j]; }
};
...
inline void Solve() { // task 1
  ...
  cout << setw(3) << "<Outer loop:" << setw(10) << outer_counter - 1
       << " times>" << endl;
}

void FindClosestDivisor(ullong k, ullong &x1, ullong &x2, ullong &target,
                        ullong &mindiff, ullong &recur_counter) {
...
Matrix fast_pow(Matrix a, ullong n, ullong &recur_counter) {
  ++recur_counter;
  if (n == 1) {
    return a;
  }
  Matrix ret = fast_pow(a, n / 2, recur_counter);
  ret = ret * ret;
  if (n % 2)
    ret = ret * a;
  return ret;
...
pUUL F(ullong n,
  ...
  if (pre.first >
      ULLONG_MAX -
          pre.second) { // if true, pre.first + pre.second will overflow <!>
  ...
  d2 = ret.first;
  mindiff = d2 - d1;
  FindClosestDivisor(sqrt(ret.first), d1, d2, ret.first, mindiff,
                     inner_counter);
...
inline void Solve2() { // task 2
  ...
  ullong Outer_Counter = 0;
  F(N, Outer_Counter);
  cout << setw(3) << "<Outer recursion:" << setw(10) << Outer_Counter
       << " times>" << endl;
}
